...
# The top-level class S3FileSystem holds connection information and allows typical file-system style operations like
# cp, mv, ls, du, glob, etc., as well as put/get of local files to/from S3.
...
# certain events have occurred. An event is described by a descriptive message w
# hich can optionally contain variable data (i.e. data that is potentially different for each occurrence of the event). 
...
# the style format '%(asctime)s - %(levelname)s - %(message)s'
